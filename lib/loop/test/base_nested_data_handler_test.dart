...
import 'package:my_analyzer/loop/src/handlers/base_nested_data_handler.dart';
...
void main() {
  group('BaseNestedDataHandler Tests', () {
    final testData = {
      'items': [
        {
          'id': 1,
    ...
    test('should return error when no items found and allowEmpty is false', () {
      final handler = BaseNestedDataHandler<Map<String, dynamic>>(allowEmpty: false);
      final result = handler.getItems(
        data: testData,
        listKey: 'items',
        conditionKey: 'id',
    ...
    test('should return empty list when no items found and allowEmpty is true', () {
      final handler = BaseNestedDataHandler<Map<String, dynamic>>(allowEmpty: true);
      final result = handler.getItems(
        data: testData,
        listKey: 'items',
        conditionKey: 'id',
    ...
    test('should transform items when transformer is provided', () {
      final handler = BaseNestedDataHandler<String>(
    ...
    test('should respect depth parameter', () {
      final handler = BaseNestedDataHandler<Map<String, dynamic>>(depth: 1);
      final result = handler.getItems(
        data: testData,
        listKey: 'items',
        conditionKey: 'id',
      ...
      expect(result.value?[0]['name'], 'Item 1.1');
    });
